...
CREATE DATABASE SmartUniversity_v1;

USE SmartUniversity_v1;
...
CREATE TABLE [User] (
    ...
    Role           NVARCHAR(20)  NOT NULL CONSTRAINT CK_User_Role CHECK (Role IN ('Admin','Instructor','Student')),
...
CREATE TABLE Instructor (
    ...
    CONSTRAINT FK_Instructor_User  FOREIGN KEY (UserID)  REFERENCES [User](UserID) ON DELETE CASCADE, -- Instructor to User
    CONSTRAINT FK_Instructor_Department FOREIGN KEY (DepartmentID) REFERENCES Department(DepartmentID) -- Instructor to Department
);
...
CREATE TABLE Student (
    ...
    CurrentLevel   INT NOT NULL CHECK (CurrentLevel BETWEEN 1 AND 4),
    ...
    CONSTRAINT FK_Student_User       FOREIGN KEY (UserID)       REFERENCES [User](UserID) ON DELETE CASCADE, -- Students to [User]
    CONSTRAINT FK_Student_Department FOREIGN KEY (DepartmentID) REFERENCES Department(DepartmentID) -- Students to Department
);
...
CREATE TABLE Course (
    ...
    CONSTRAINT FK_Course_Department FOREIGN KEY (DepartmentID) REFERENCES Department(DepartmentID),-- Course to Department
    CONSTRAINT FK_Course_Instructor FOREIGN KEY (InstructorID) REFERENCES Instructor(InstructorID) -- Course to Instructor
);
...
CREATE TABLE Registration (
    ...
    Status NVARCHAR(15) NOT NULL DEFAULT 'enrolled'CHECK (Status IN ('enrolled','dropped','completed')),
    CONSTRAINT UQ_Registration UNIQUE (StudentID, CourseID, SemesterID),
    CONSTRAINT FK_Reg_Student  FOREIGN KEY (StudentID)  REFERENCES Student(StudentID)  ON DELETE CASCADE, --Registration to Student
    CONSTRAINT FK_Reg_Course   FOREIGN KEY (CourseID)   REFERENCES Course(CourseID),--Registration to Course
    CONSTRAINT FK_Reg_Semester FOREIGN KEY (SemesterID) REFERENCES Semester(SemesterID) --Registration to Semester
);
...
CREATE TABLE [Session] (
    ...
    QRExpiry     DATE NOT NULL,
    CONSTRAINT CK_Session_Time CHECK (EndTime > StartTime),
    CONSTRAINT FK_Session_Course     FOREIGN KEY (CourseID)     REFERENCES Course(CourseID), -- Sessions to Course
    CONSTRAINT FK_Session_Instructor FOREIGN KEY (InstructorID) REFERENCES Instructor(InstructorID), -- Sessions to  Instructor
    CONSTRAINT FK_Session_Semester   FOREIGN KEY (SemesterID)   REFERENCES Semester(SemesterID) -- Sessions to Semester
);
...
CREATE TABLE Attendance (
    ...
    CONSTRAINT FK_Att_Student FOREIGN KEY (StudentID) REFERENCES Student(StudentID) ON DELETE CASCADE, -- Attendance to Student
    CONSTRAINT FK_Att_Session FOREIGN KEY (SessionID) REFERENCES [Session](SessionID) ON DELETE CASCADE -- Attendance to [Session]
);
...
CREATE TABLE Exam (
    ...
    CONSTRAINT FK_Exam_Course   FOREIGN KEY (CourseID)   REFERENCES Course(CourseID), -- Exam to Course
    CONSTRAINT FK_Exam_Semester FOREIGN KEY (SemesterID) REFERENCES Semester(SemesterID) -- Exam to Semester
);
...
CREATE TABLE Grade (
    ...
    CONSTRAINT FK_Grade_Registration FOREIGN KEY (RegistrationID) REFERENCES Registration(RegistrationID) ON DELETE CASCADE -- Grades to Registration
);
...
CREATE TABLE SemesterGPA (
    ...
    CONSTRAINT FK_SGPA_Student  FOREIGN KEY (StudentID)  REFERENCES Student(StudentID)  ON DELETE CASCADE, -- SemesterGPA to Student 
    CONSTRAINT FK_SGPA_Semester FOREIGN KEY (SemesterID) REFERENCES Semester(SemesterID) -- SemesterGPA to Semester
);
...
/* -------------------------
   Semesters (FIXED: IsActive added)
------------------------- */
...
/* -------------------------
   Students (FIXED: RiskLevel + correct order)
------------------------- */
INSERT INTO Student(
    UserID,DepartmentID,NationalID,Gender,
    Nationality,BirthDate,Mobile,
    EnrollmentYear,CurrentLevel,CGPA,AttendanceRate,RiskLevel
)
SELECT 
    u.UserID,
    d.DepartmentID,
...
/* -------------------------
   Active Semester
------------------------- */
DECLARE @ActiveSem INT =
(SELECT SemesterID FROM Semester WHERE IsActive = 1);
...
INSERT INTO [Session](
    ...
    SessionType,QRCode,QRExpiry
)
SELECT c.CourseID, c.InstructorID, @ActiveSem,
       '2026-05-01','09:00','10:30',
       'Lecture','QR-CS101-DEMO-001','2026-05-01T10:30:00'
FROM Course c
WHERE c.Code = 'CS101';
